...
const registerUser = async (req, res) => {
    try {
        ...
        const isAlreadyExist = await userModels.findOne({ $or: [{ email }, { username }] });
        if (isAlreadyExist) {
...
const getMe = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "User fetched successfully", user: sanitizeUser(user) });
    } catch (error) {
        console.error(error);
...
const getRecommendedArtists = async (req, res) => {
    try {
        const artists = ["Arijit Singh", "KK", "Shreya Ghoshal", "Atif Aslam", "Anuv Jain"];
...
module.exports = { registerUser, loginUser, logoutUser, getMe, logMood, logSong, getRecommendedArtists };
